...
from models import (
    User, UserScoreHistory, Meeting, MeetingStatus, HandicapUpdateMethod,
    Score, MeetingParticipant, MeetingResult
)
...
def get_user_handicap_for_formation(db: Session, user_id: int) -> Optional[Decimal]:
    ...
    actual_score_count = db.query(UserScoreHistory).filter(
        UserScoreHistory.user_id == user_id
    ).count()
    
    # 경기 기록이 1경기 이상이고 calculated_handicap이 있으면 사용
    # (handicap_calculation_count가 0이어도 실제 데이터가 있으면 사용)
    if actual_score_count >= 1 and user.calculated_handicap is not None:
        return user.calculated_handicap
    
    # 경기 기록이 없거나 calculated_handicap이 없으면 initial_handicap 사용
    if user.initial_handicap is not None:
        return user.initial_handicap
...
def update_user_handicap(
    ...
    # 사용자 정보 업데이트
    user.calculated_handicap = calculated_handicap
...
def save_score_to_history(
    ...
    meeting_id: int,
    gross_score: int,
    handicap_used: Decimal,
    played_at: datetime
) -> Optional[UserScoreHistory]:
    """
    ...
    existing = db.query(UserScoreHistory).filter(
        UserScoreHistory.user_id == user_id,
...
def process_meeting_completion(
    db: Session,
    meeting_id: int,
    score_count: int = 5
) -> dict:
    ...
    for participant in participants:
        try:
            updated_handicap = process_participant_score(
                db=db,
...
def create_meeting_results(
    ...
    existing_results = db.query(MeetingResult).filter(
    ...
    for result in participant_results:
        try:
            # 중복 확인 (같은 모임, 같은 사용자)
            existing = db.query(MeetingResult).filter(
                MeetingResult.meeting_id == meeting_id,
                MeetingResult.user_id == result["user_id"]
            ).first()
            ...
            score_history = db.query(UserScoreHistory).filter(
                UserScoreHistory.user_id == result["user_id"],
                UserScoreHistory.meeting_id == meeting_id
            ).first()
            
            if not score_history:
                logger.warning(f"UserScoreHistory가 없어 MeetingResult를 생성할 수 없습니다: user_id={result['user_id']}, meeting_id={meeting_id}")
                continue
