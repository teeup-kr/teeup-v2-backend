...
class TeamFormationEngine:
    ...
    def classify_by_handicap(self, participants: List[MeetingParticipant]) -> Dict[str, List[MeetingParticipant]]:
        ...
        for participant in participants:
            handicap = float(participant.handicap_index or 0)
            if handicap <= 9:
    ...
    def calculate_team_score(self, team_members: List[MeetingParticipant], 
        ...
        # 1. 실력 균형 점수 (0-100)
        handicaps = [float(m.handicap_index or 0) for m in team_members]
    ...
    #                                key=lambda p: float(p.handicap_index or 0))
    #     
    #     teams = []
    #     i = 0
    #     
    ...
    #         base_handicap = float(sorted_participants[i].handicap_index or 0)
    #         
    ...
    #                          key=lambda v: abs(float(v.handicap_index or 0) - float(newbie.handicap_index or 0)))
    #         
    ...
    def create_teams_from_formation(self, meeting_id: int, 
        ...
        # 같은 파일에 있는 함수이므로 직접 호출
        team_groups = form_teams_by_mode(
        ...
        for i, team_members in enumerate(team_groups):
            ...
            for member in team_members:
                if member.is_guest:
                    ...
                    elif member.guest_handicap:
                        total_handicap += float(member.guest_handicap)
                else:
                    ...
                    if member.user_id:
                        ...
                        elif member.handicap_index:
                            total_handicap += float(member.handicap_index)
        ...
        for team in teams:
            for member in team.members:
                ...
                participant = self.db.query(MeetingParticipant).filter(
                    ...
                    MeetingParticipant.user_id == member.user_id
                ).first()
                
                if participant and participant.handicap_index:
                    handicaps.append(float(participant.handicap_index))
...
def zigzag_distribute(participants: List[MeetingParticipant], team_count: int, max_team_size: int = 4) -> List[List[MeetingParticipant]]:
    """
...
def sort_by_previous_record(participants: List[MeetingParticipant], db: Session) -> List[MeetingParticipant]:
    ...
    def get_net_score_value(participant: MeetingParticipant) -> Tuple[bool, float]:
        ...
        if participant.user_id:
            ...
            latest_result = db.query(MeetingResult).filter(
...
def form_teams_by_mode(
    participants: List[MeetingParticipant],
    formation_mode: TeamFormationMode,
    db: Session,
    max_team_size: int = 4
) -> List[List[MeetingParticipant]]:
    """
    ...
    if formation_mode in [
        TeamFormationMode.GENDER_SEPARATED_HANDICAP,
        TeamFormationMode.GENDER_SEPARATED_PREVIOUS_RECORD,
        ...
        if gender_groups["male"]:
            male_teams = _form_teams_by_criteria(
                gender_groups["male"],
                formation_mode,
                db,
                max_team_size
        ...
        if gender_groups["female"]:
            female_teams = _form_teams_by_criteria(
                gender_groups["female"],
                formation_mode,
                db,
                max_team_size
            )
        ...
        if gender_groups["other"]:
            other_teams = _form_teams_by_criteria(
                gender_groups["other"],
                formation_mode,
                db,
                max_team_size
    ...
    # 성별 혼합 모드
        ...
        # 전체 참가자에 대해 편성
        all_teams = _form_teams_by_criteria(
            participants,
            formation_mode,
            db,
            max_team_size
...
def _form_teams_by_criteria(
    ...
    if formation_mode in [
        TeamFormationMode.GENDER_SEPARATED_RANDOM,
        TeamFormationMode.GENDER_MIXED_RANDOM
    ]:
    ...
    if formation_mode in [
        TeamFormationMode.GENDER_SEPARATED_HANDICAP,
        TeamFormationMode.GENDER_MIXED_HANDICAP
    ]:
        # 핸디캡 기준 정렬
        sorted_participants = sort_by_handicap(participants, db)
    elif formation_mode in [
        TeamFormationMode.GENDER_SEPARATED_PREVIOUS_RECORD,
        TeamFormationMode.GENDER_MIXED_PREVIOUS_RECORD
    ]:
        # 직전대회 성적 기준 정렬
...
def add_guest_to_meeting(
    ...
    guest = Guest(
        name=guest_name,
        handicap=guest_handicap,
        birthdate=guest_birthdate_dt,
        gender=guest_gender
    )
    ...
    guest_participant = MeetingParticipant(
        ...
        is_guest=True,
        # 하위 호환성을 위해 게스트 필드도 채움
        guest_name=guest_name,
        guest_handicap=guest_handicap,
        guest_birthdate=guest_birthdate_dt,
