...
from schemas import (
    RoundingMeetingCreate, MeetingUpdate, 
    MeetingResponse, MeetingParticipantResponse, PaginatedResponse,
    TeamResponse, TeamMemberResponse, TeamStatus
)
from routers.auth import get_current_active_user
from routers.auth import get_current_admin_user_jwt, get_current_user_or_admin
...
async def get_rounds(
    ...
    for meeting in meetings:
        ...
        if len(meeting_responses) == 0:
            logger.info(f"🔍 첫 번째 모임 참가자 수 - meeting_id: {meeting.id}, name: {meeting.name}, participant_count: {participant_count}")
    ...
    logger.info(f"🔍 라운딩 모임 응답 - total: {total}, total_pages: {calculated_total_pages}, meetings_count: {len(meeting_responses)}")
        ...
        for participant in participants:
            ...
            if participant.guest_id:
                ...
                participant_responses.append({
                    ...
                    "role": participant.role.value if hasattr(participant.role, 'value') else str(participant.role),
                    "handicap_index": int(guest_handicap) if guest_handicap else None,
                    "handicap": float(guest_handicap) if guest_handicap else None,
                    "recent_avg_score": None,  # 게스트는 직전 대회 성적 없음
        ...
        for team in teams:
            ...
            for team_member in team_members:
                ...
                if team_member.user_id:
                    participant = db.query(MeetingParticipant).filter(
                        MeetingParticipant.meeting_id == meeting_id,
                        MeetingParticipant.user_id == team_member.user_id
                    ).first()
                ...
                if participant:
                    ...
                    member_response = TeamMemberResponse(
                        id=team_member.id,                        team_id=team_member.team_id,
...
async def add_team_member(
    ...
    try:
        ...
        if not user_id and not guest_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="user_id 또는 guest_id가 필요합니다."
            )
        ...
        if user_id:
            ...
            participant = db.query(MeetingParticipant).filter(
                and_(
                    MeetingParticipant.meeting_id == meeting_id,
                    MeetingParticipant.guest_id == guest_id,
                    MeetingParticipant.status == MeetingParticipantStatus.CONFIRMED
                )
            ).first()
        else:
        ...
        if not participant:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="해당 사용자는 이 라운딩의 참가자가 아닙니다."
            )
        ...
        if existing_member:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="이미 해당 팀의 멤버입니다."
            )
        
        # 기존 멤버 수 확인하여 order 설정
        existing_members_count = db.query(TeamMember).filter(
            TeamMember.team_id == team_id
        ).count()
        ...
        team_member = TeamMember(
            ...
            user_id=user_id,
            guest_id=guest_id,
            order=existing_members_count + 1
        )
