...
async def create_meeting(
    meeting_data: RoundingMeetingCreate,
    ...
    try:
        ...
        if not membership:
            raise HTTPException(
                ...
                detail="클럽 멤버만 모임을 생성할 수 있습니다."
            )
        
        # 라운딩 모임은 리더/매니저만 생성 가능, 이벤트는 일반 멤버도 가능
        if meeting_data.meeting_type == MeetingType.ROUND and membership.role not in [ClubRole.LEADER, ClubRole.MANAGER]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="라운딩 모임은 클럽 리더/매니저만 생성할 수 있습니다."
            )
...
async def get_rounding_meetings(
...
async def get_meeting(
    ...
    try:
        ...
        meeting_with_club = db.query(Meeting, Club).join(
            Club, Meeting.club_id == Club.id
        ...
        for participant, user_realname, user_nickname in participants_with_users:
            participant_data.append({
                ...
                "tee_preference": participant.tee_preference if participant.tee_preference is not None else "",
                "is_newbie": participant.is_newbie if participant.is_newbie is not None else False,
                "prefer_with": participant.prefer_with if participant.prefer_with is not None else [],
                "avoid_with": participant.avoid_with if participant.avoid_with is not None else [],
...
async def update_meeting(
    ...
    try:
        ...
        if not is_meeting_organizer_or_manager(meeting_id, current_user.id, db):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="모임 매니저만 모임 정보를 수정할 수 있습니다."
            )
    ...
    try:
        ...
        if notification_data.send_to_all_participants:
            ...
            if not notification_data.participant_ids:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="알림을 받을 참가자 ID 목록이 필요합니다."
                )
        ...
        for user_id in target_user_ids:
            try:
                notification = Notification(
                    user_id=user_id,
                    type=NotificationType(notification_data.notification_type).value if hasattr(NotificationType(notification_data.notification_type), 'value') else str(NotificationType(notification_data.notification_type)),
        ...
        response = MeetingNotificationResponse(
            notification_id=notification_ids[0] if notification_ids else "",
            title=notification_data.title,
            content=notification_data.content,
...
모임에 참석할 준비를 해주세요!
        ...
        return {
            "message": f"{sent_count}명에게 모임 리마인더가 전송되었습니다.",
            "success": True
        }
