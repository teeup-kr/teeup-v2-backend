...
from models import (
    Team, TeamMember, MeetingParticipant, Meeting, User, MeetingResult, Guest
)
from schemas import (
    TeamFormationMode, TeamStatus, MeetingParticipantStatus,
    ClubRole
)
from schemas import (
    TeamCreate, TeamUpdate, TeamResponse, TeamMemberResponse,
    TeamFormationRequest, TeamFormationResponse, MessageResponse
)
...
async def get_team(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"팀 조회 중 오류가 발생했습니다: {str(e)}"
        )
...
async def delete_team(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"팀 삭제 중 오류가 발생했습니다: {str(e)}"
        )
...
async def auto_form_teams(
    ...
    if len(participants) < 2:
        raise HTTPException(
            ...
            detail="팀 편성을 위해서는 최소 2명의 확정된 참가자가 필요합니다"
        )
