...
from models import Admin, User, Meeting, ClubMembership, Club, Terms, UserStatus, ClubStatus, ClubRole
...
async def generate_client_token(
    ...
    try:
        ...
        # 5분 유효기간의 임시 토큰 생성
        temp_token = jwt_auth.create_access_token(
            user_data,
...
def admin_list_clubs(
    ...
    try:
        ...
        total = query.count()
        items = (
            query.order_by(Club.created_at.desc())
            .offset((page - 1) * limit)
            .limit(limit)
            .all()
        )
        ...
        for c in items:
            ...
            try:
                leader_membership = (
                    ...
                    .filter(ClubMembership.club_id == c.id, ClubMembership.role == ClubRole.LEADER)
                    .first()
                )
            ...
            data.append({
                "id": c.id,                "display_id": getattr(c, 'display_id', None),
                "name": c.name,
                "description": c.description,
                "type": c.type.value if hasattr(c.type, 'value') else str(c.type),
                "member_count": actual_member_count,  # 실제 멤버 수 사용
                "location": getattr(c, 'location', None),
                "representative_name": getattr(c, 'representative_name', None),
...
async def get_user_meetings(
    ...
    try:
        ...
        # 라운딩 모임 조회
        rounding_query = db.query(MeetingParticipant, Meeting, Club).join(
        ...
        rounding_results = rounding_query.order_by(
            desc(func.coalesce(Meeting.meeting_time, Meeting.created_at))
        ).offset(offset).limit(limit).all()
        
        # 소셜 모임 조회
        social_query = db.query(MeetingParticipant, Meeting, Club).join(
            Meeting, MeetingParticipant.meeting_id == Meeting.id
        ).join(
            Club, Meeting.club_id == Club.id
        ).filter(
        ...
        social_results = social_query.order_by(
            desc(func.coalesce(Meeting.meeting_time, Meeting.created_at))
        ).offset(offset).limit(limit).all()
        ...
        for participant, meeting, club in rounding_results:
            ...
            rounding_meetings.append(schemas.UserMeetingItem(
        ...
        for participant, meeting, club in social_results:
            social_meetings.append(schemas.UserMeetingItem(
                meeting_id=meeting.id,
                meeting_name=meeting.name,
                club_name=club.name,
                meeting_time=meeting.meeting_time,
...
async def get_user_handicap_history(
    ...
    try:
        ...
        # 스코어 히스토리 조회
        score_history_query = db.query(UserScoreHistory, Meeting, Club).join(
            Meeting, UserScoreHistory.meeting_id == Meeting.id
        ).join(
            Club, Meeting.club_id == Club.id
        ).filter(
            UserScoreHistory.user_id == user_id,
        ...
        results = score_history_query.order_by(
            desc(UserScoreHistory.played_at)
        ).offset(offset).limit(limit).all()
        
        # 스코어 히스토리 데이터 구성
        score_history = []
        for score_history_item, meeting, club in results:
            score_history.append(schemas.HandicapHistoryItem(
...
async def get_admin_dashboard(
    ...
    try:
        ...
        # 최근 소셜 모임 생성 (최근 5개)
        recent_socials = db.query(Meeting).filter(
            Meeting.meeting_type == MeetingType.SOCIAL
        ...
        return {
            ...
            "user": {
                "id": current_user.get("id"),                "email": current_user.get("email"),
                "role": current_user.get("role")
            }
        }
    except Exception as e:
...
async def get_dashboard_stats(
    ...
    try:
        ...
        weekly_rounds = db.query(Meeting).filter(
            Meeting.meeting_type == MeetingType.ROUND,
            Meeting.created_at >= start_of_week
        ).count()
...
async def get_admin_club(
    ...
    try:
        ...
        # 현재 리더 정보 조회
        current_leader = db.query(ClubMembership).filter(
            ClubMembership.club_id == club.id,
        ...
        return {
            "id": club.id,            "display_id": club.display_id,
            "name": club.name,
            "type": club.type.value if hasattr(club.type, 'value') else str(club.type),
            "description": club.description,
            "status": club.status.value if hasattr(club.status, 'value') else str(club.status),
            "representative_name": club.representative_name,
            "representative_id": current_leader.user_id if current_leader else None,
            "location": club.location,
            "contact_info": club.contact_info,
            "additional_info": club.additional_info,
            "profile_image": club.profile_image,
            "member_count": club.member_count,  # 예상 멤버 수 (레거시)
            "current_member_count": current_member_count,  # 실제 멤버 수
...
async def create_admin_club(
    ...
    """관리자용 클럽 생성"""
    from models import User, Club, ClubMembership, ClubRole, ClubStatus
    from schemas import MembershipStatus
    from utils.display_id_generator import generate_club_display_id
    ...
    try:
        ...
        if not representative_id:
            raise HTTPException(
                ...
                detail="대표자 사용자 ID가 필요합니다."
            )
        
        representative_user = db.query(User).filter(
        ...
        representative_name = (
            getattr(representative_user, 'realname', None) 
            or representative_user.nickname 
        ...
        regular_fee_response = db.query(ClubFeeModel).filter(
            ClubFeeModel.club_id == club.id,
            ClubFeeModel.is_active == True,
            ClubFeeModel.cycle.in_([BillingCycle.MONTHLY, BillingCycle.QUARTERLY, BillingCycle.YEARLY])
        ).first()
        ...
        return {
            ...
            "has_regular_fee": regular_fee_response is not None,
            "regular_fee_amount": float(regular_fee_response.amount) if regular_fee_response and regular_fee_response.amount else None,
            "regular_fee_cycle": regular_fee_response.cycle.value if regular_fee_response and regular_fee_response.cycle else None,
            "regular_fee_description": regular_fee_response.description if regular_fee_response else None,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"클럽 생성 중 오류가 발생했습니다: {str(e)}"
        )
...
async def admin_upload_file(
    ...
    try:
        ...
        if not validate_file(file):
            raise HTTPException(
                status_code=400,
                detail="지원하지 않는 파일 형식입니다. PNG, JPG, JPEG, PDF만 업로드 가능합니다."
            )
...
async def get_admin_club_pending_members(
    club_id: str,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_admin_user)
):
...
async def get_admin_club_members(
    ...
    try:
        ...
        query = db.query(ClubMembership, User).join(
        ...
        for membership, user in results:
            ...
            member = {
                "id": membership.id,                "club_id": membership.club_id,
...
async def approve_admin_club_membership(
    club_id: str,
    user_id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_admin_user)
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"가입 승인 중 오류가 발생했습니다: {str(e)}"
        )

@router.put("/clubs/{club_id}/members/{user_id}/reject")
async def reject_admin_club_membership(
    club_id: str,
    user_id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_admin_user)
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"가입 거절 중 오류가 발생했습니다: {str(e)}"
        )

@router.post("/clubs/{club_id}/members")
async def add_admin_club_member(
    club_id: str,
    user_id: int,
    role: str = "MEMBER",
    db: Session = Depends(get_db),
    ...
    try:
        ...
        if existing_membership:
            if existing_membership.status == MembershipStatus.ACTIVE:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="이미 해당 클럽의 멤버입니다"
                )
            else:
                # 승인되지 않은 상태라면 승인 상태로 변경
                existing_membership.status = MembershipStatus.ACTIVE
                existing_membership.role = ClubRole[role] if role in ['LEADER', 'MANAGER', 'MEMBER'] else ClubRole.MEMBER
...
async def update_admin_club_member_role(
    club_id: str,
    user_id: int,
    role_data: dict,
    db: Session = Depends(get_db),
    ...
    try:
        ...
        return {
            "message": f"멤버 역할이 {new_role}로 변경되었습니다",
            "success": True
        }
...
async def remove_admin_club_member(
    club_id: str,
    user_id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_admin_user)
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"멤버 내보내기 중 오류가 발생했습니다: {str(e)}"
        )

@router.put("/clubs/{club_id}")
async def update_admin_club(
    club_id: str,
    club_data: dict,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_admin_user)
...
async def delete_admin_club(
    ...
    try:
        ...
        # 클럽에 승인된 멤버가 있는지 확인
        approved_members = db.query(ClubMembership).filter(
            ClubMembership.club_id == club.id,
            ClubMembership.status == MembershipStatus.ACTIVE
        ).count()
        ...
        return {
            "success": True,
            "message": f"클럽 '{club.name}'이 성공적으로 삭제되었습니다."
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"클럽 삭제 중 오류가 발생했습니다: {str(e)}"
        )

@router.get("/meetings/rounding")
async def admin_get_rounding_meetings(
    ...
    try:
        ...
        for meeting in meetings:
            ...
            meeting_data.append({
                "id": str(meeting.id),                "name": str(meeting.name) if meeting.name else "",
                "description": str(meeting.description) if meeting.description else "",
                "meeting_type": str(meeting_type_str) if meeting_type_str else "",
                "meeting_subtype": str(meeting_subtype_str) if meeting_subtype_str else "",
                "location": str(meeting.location) if meeting.location else "",
                "course_name": str(meeting.course_name) if meeting.course_name else "",
                ...
                "status": str(status_str) if status_str else "",
                "club_id": str(meeting.club_id),
                "club_name": str(meeting.club.name) if meeting.club and meeting.club.name else "",
        ...
        raise HTTPException(
            ...
            detail="라운딩 모임 목록 조회 중 오류가 발생했습니다"
        )

@router.get("/meetings/event")
async def admin_get_event_meetings(
    page: int = Query(1, ge=1, description="페이지 번호"),
    limit: int = Query(10, ge=1, le=100, description="페이지당 항목 수"),
    status: Optional[str] = Query(None, description="모임 상태 필터"),
    search: Optional[str] = Query(None, description="검색어"),
    ...
    try:
        ...
        for meeting in meetings:
            ...
            meeting_data.append({
                ...
                "meeting_subtype": meeting.meeting_subtype.value if hasattr(meeting.meeting_subtype, 'value') else str(meeting.meeting_subtype),
        ...
        raise HTTPException(
            status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="이벤트 모임 목록 조회 중 오류가 발생했습니다"
        )

@router.get("/meetings/{meeting_id}")
async def get_admin_meeting(
    meeting_id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_admin_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="모임 상세 조회 중 오류가 발생했습니다"
        )

@router.post("/meetings/rounding")
async def create_admin_rounding_meeting(
    meeting_data: dict,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_admin_user)
):
    ...
    try:
        ...
        meeting = Meeting(
            ...
            meeting_time=datetime.fromisoformat(meeting_data.get('meeting_time').replace('Z', '+00:00')) if meeting_data.get('meeting_time') else None,
            tee_time=datetime.strptime(meeting_data.get('tee_time'), '%H:%M').time() if meeting_data.get('tee_time') and ':' in meeting_data.get('tee_time') else None,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="라운딩 모임 생성 중 오류가 발생했습니다"
        )

@router.post("/meetings/event")
async def create_admin_event_meeting(
    meeting_data: dict,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_admin_user)
):
...
async def admin_cancel_payment(
    ...
    try:
        ...
        socials_count = db.query(Meeting).filter(
            Meeting.meeting_type == MeetingType.SOCIAL,
            Meeting.club_id.in_(
                db.query(ClubMembership.club_id).filter(
                    ClubMembership.user_id == user_id
                )
            ),
...
async def delete_admin_plan(
    ...
    try:
        ...
        if active_subscriptions > 0:
            raise HTTPException(
                status_code=400,
                detail="활성 구독이 있는 플랜은 삭제할 수 없습니다"
            )
...
async def create_admin_terms(
    ...
    try:
        ...
        if terms_data.get('is_active', True):
            db.query(Terms).filter(
                Terms.type == TermsType(terms_data.get('type', 'SERVICE'))
        ...
        return {
            "id": new_term.id,            "title": new_term.title,
...
async def create_admin_notice(
    ...
    try:
        ...
        return {
            "id": new_notice.id,            "title": new_notice.title,
...
async def get_admin_inquiries(
    ...
    try:
        ...
        for inquiry in inquiries:
            inquiry_list.append({
                "id": inquiry.id,                "title": inquiry.title,
...
async def get_admin_inquiry_detail(
    ...
    try:
        ...
        ).order_by(InquiryResponse.created_at).all()
...
async def update_admin_inquiry_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="문의 상태 업데이트 중 오류가 발생했습니다"
        )
    ...
    try:
        ...
        for participant in participants:
            participant_list.append({
                "id": participant.id,                "meeting_id": participant.meeting_id,
...
async def update_admin_participant_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="모임 참가자 상태 업데이트 중 오류가 발생했습니다"
        )
...
async def get_admin_profile(
    ...
    try:
        ...
        logger.info(f"Admin profile request - user_id: {user_id}")
        
        if not user_id:
            logger.error("Admin profile request - No user_id in current_user")
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="인증이 필요합니다"
            )
        ...
        user = db.query(User).filter(User.id == user_id, User.deleted_at.is_(None)).first()
        logger.info(f"Admin profile request - user found: {user is not None}")
        
        if not user:
            logger.error(f"Admin profile request - User not found for id: {user_id}")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="사용자를 찾을 수 없습니다"
            )
...
async def update_admin_profile(
    ...
    try:
        ...
        if 'name' in profile_data:
            user.realname = profile_data['name']
