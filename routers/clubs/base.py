...
from schemas import (
    ClubCreate, ClubUpdate, ClubResponse, ClubMembersResponse, 
    ClubMembershipResponse,
    PaginatedResponse, MessageResponse,
    ClubRole, ClubStatus, MembershipStatus
)
...
async def get_clubs(
    ...
    try:
        ...
        for club in clubs:
            ...
            # 현재 사용자의 멤버십 상태 확인
            user_membership = db.query(ClubMembership).filter(
                ClubMembership.club_id == club.id,
            ...
            if user_membership:
                membership_status = user_membership.status.value if hasattr(user_membership.status, "value") else user_membership.status
                membership_role = user_membership.role.value if hasattr(user_membership.role, "value") else user_membership.role
...
async def get_my_clubs(
    ...
    try:
        ...
        if status_filter == 'PENDING':
            ...
            query = db.query(ClubMembership, Club).join(
        ...
        for membership, club in results:
            ...
            club_data = {
                ...
                "membership_status": membership.status.value if hasattr(membership.status, "value") else membership.status,
...
async def get_club(
    ...
    try:
        ...
        if membership:
            club.membership_status = membership.status.value if hasattr(membership.status, "value") else str(membership.status)
        ...
        try:
            ...
            if leader_membership:
                ...
                if leader_user:
                    ...
                    logger.info(f"클럽 {club.id} ({club.name}) 대표자명 업데이트: '{original_representative_name}' -> '{new_representative_name}' (리더: user_id={leader_user.id}, email={leader_user.email})")
                else:
                    logger.warning(f"클럽 {club.id} ({club.name}) 리더 멤버십은 있지만 사용자를 찾을 수 없음: user_id={leader_membership.user_id}")
            else:
        ...
        if not admin:
            ...
            if not membership:
                raise HTTPException(
                    ...
                    detail="클럽 리더나 매니저만 클럽 정보를 수정할 수 있습니다."
                )
...
async def check_active_meetings(
    club_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
):
    ...
    try:
        ...
        if not membership:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="클럽 리더나 매니저만 활성 모임을 확인할 수 있습니다."
            )
        ...
        # NOTE: Meeting 모델에 deleted_at이 없으므로 상태만 체크
        active_events = db.query(Meeting).filter(
            Meeting.club_id == club.id,
